...
abstract contract EsterSpookyStrategyBase is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public estReward = 0;
    uint256 public constant estRewardUL = 800;
    
    uint256 public callerReward = 0;
    uint256 public constant callerRewardUL = 100;
    ...
    event SetBuyBackRouterAddress(address _buyBackRouterAddress);
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(estFarmAddress, _wantAmt);
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        earnedAmt = chargeFees(earnedAmt);
    ...
    function chargeFees(uint256 _earnedAmt) internal virtual returns (uint) {
        uint totalFee = buyBackRate.add(estReward).add(callerReward).add(controllerFee);
        uint toWftm = _earnedAmt.mul(totalFee).div(max_fee);
        ...
        uint toCaller = wftmBal.mul(callerReward).div(totalFee);
        uint toEstReward = wftmBal.mul(estReward).div(totalFee);
        IERC20(wftm).safeTransfer(msg.sender, toCaller);
        IERC20(wftm).safeTransfer(rewardsAddress, toEstReward);

        uint buyBack = wftmBal.mul(buyBackRate).div(totalFee);
        ...
        if (controllerFee > 0) {
            IERC20(wftm).safeTransfer(devAddress, toWftm.sub(toCaller).sub(toEstReward).sub(buyBack));
        }

        return _earnedAmt.sub(toWftm);
    }
    ...
    function setSettings(
        ...
        require(_estReward <= estRewardUL, "_estReward too high");
        estReward = _estReward;

        require(_callerReward <= callerReward, "_callerReward too high");
        ...
        onlyAllowGov
    {
        ...
        emit SetBuyBackRouterAddress(_buyBackRouterAddress);
    }
    ...
    function _wrapFTM() internal virtual {
        // FTM -> WFTM
        uint256 ftmBal = address(this).balance;
        if (ftmBal > 0) {
            IWFTM(wftm).deposit{value: ftmBal}(); // FTM -> WFTM
        }
    }

    function wrapFTM() public virtual onlyAllowGov {
        _wrapFTM();
    }
...
contract EsterSpookyStrategy is EsterSpookyStrategyBase {
    constructor(
        ...
        buyBackRouterAddress = _addresses[11];
        entranceFeeFactor = _entranceFeeFactor;
        withdrawFeeFactor = _withdrawFeeFactor;

        transferOwnership(estFarmAddress);
    }
}
